...
1. **查询每个学生的姓名、总成绩、平均成绩**（要求：未选课的学生也要显示，总成绩为 0）
2. **查询所有课程中，最高分低于该课程平均分的课程名称**
3. **用事务实现：插入一条新订单，并同时更新库存表，如果库存不足则回滚**
4. **为 order 表的 user_id 和 create_time 创建联合索引，并说明什么查询能用到该索引**

---

如果你想，我可以：
- 逐一给你写出上述练习题的参考答案
- 或者讲解 **执行计划（EXPLAIN）** 的详细解读
- 或者梳理 **MySQL 存储引擎（InnoDB vs MyISAM）** 的区别
